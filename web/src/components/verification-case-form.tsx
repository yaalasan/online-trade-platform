    ...
    <form action={onSubmit} className="space-y-4">
      <fieldset disabled={readOnly || pending} className="space-y-4">
        <div>
          ...
          <Input id="legalName" name="legalName" defaultValue={values.legalName} placeholder="Shenzhen Acme Manufacturing Co., Ltd." />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label htmlFor="registrationNumber">Registration / USCC no.</Label>
            <Input id="registrationNumber" name="registrationNumber" defaultValue={values.registrationNumber} placeholder="91440300MA5…" />
          </div>
          <div>
            <Label htmlFor="registeredCountry">Country (ISO-2)</Label>
      ...
      {readOnly && (
        <p className="text-sm text-neutral-500">The application is locked while under review.</p>
      )}
      {msg && (
